...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        # 2. connect the nodes
        # for example, every A node in the previous level has a list of S nodes in effnodes that represent the effect produced by the action. 
        #  These literals will all be part of the new S level.  Since we are working with sets,
        #  they may be "added" to the set without fear of duplication.  # However, it is important to then correctly create and connect all of the new S nodes as children of all the A nodes that could produce them, and likewise add the A nodes to the parent sets of the S nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''if PgNode_a.action negates an effect_other:
            return True
        else:
            return False        
        return False'''
        ...
        for effect in node_a2.action.effect_rem:
            if effect in node_a1.action.predond_pos:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''if effect_action is neg_precondition:
            return True
        else:
            return False
        return False'''
        ...
        for posprecond in self.current_state_links_pos:
            negeff = posprecond
            if negeff in self.next_state_links_neg:
               for a in current_state_links_pos[posprecond]:
                  for b in current_state_links_neg[negeff]:
                      if set([a, b]) not in self.mutex:
                         self.mutex.append(set([a, b])
         ...
         for negprecond in self.current_state_links_neg:
            ...
            if poseff in self.next_state_links_pos:
               for a in current_state_links_pos[poseff]:
                  for b in current_state_links_neg[negprecond]:
                      if set([a, b]) not in self.mutex:
                         self.mutex.append(set([a, b])'''
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''if precondition_action is mutex_precondition_other_action:
            return True
        else:
            return False
        return False'''
        ...
        for posprecond in self.current_state_links_pos:
            ...
            if negprecond in self.current_state_links_neg:                              
                for a in self.current_state_links_pos[posprecond]:
                    for b in self.current_state_links_net[negprecond]:                       
                        if set([a, b]) not in self.mutex:                   
                           self.mutex.append(set([a ,b]))'''
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        '''def __eq__(node_s1, node_s2):
            return (isinstance(node_s2, self.__class__) and
                    node_s1.is_pos == node_s2.is_pos and
                    node_s1.symbol == node_s2.symbol)
        return False'''
        
        same_sym = node_s1.symbol == node_s2.symbol
        neg_each = node_s1.is_pos != node_s2.is_pos
        
        return same_sym and neg_each
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        '''def is_mutex(node_s1, node_s2):
            if node_s2 in self.mutex:
                return True
        return False'''
        '''
        state_mutex = []
        for pair in self.mutex:                                   
         ...
         if (len(next_state_0) == 1) and (len(next_state_1) == 1):                                  
            state_mutex_append(set([next_state_0[0], next_state_1[0])'''
